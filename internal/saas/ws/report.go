...
type InstanceFiller interface {
	ApplyFillReport(in instance.FillInput) error
...
func (b *FillBridge) OnFill(report protocol.FillReport) {
	...
	if status == "failed" {
		if err := b.Inst.ApplyFillReport(in); err != nil {
			b.Log.Warn("record failed fill",
	...
	if err := b.Inst.ApplyFillReport(in); err != nil {
		b.Log.Warn("ApplyFillReport failed",
			zap.Uint("instance_id", report.InstanceID),
			zap.String("client_order_id", report.ClientOrderID),
			zap.Error(err),
		)
		_ = b.Inst.MarkError(report.InstanceID, "ApplyFillReport: "+err.Error())
		return
	}
